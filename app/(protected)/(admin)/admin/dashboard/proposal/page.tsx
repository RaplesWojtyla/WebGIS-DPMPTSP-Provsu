...
export default function AdminProposalPage() {
    ...
    const [stats, setStats] = useState({ submitted: 0, verified: 0, revision: 0, approved: 0, rejected: 0, total: 0 })
    ...
    const loadData = async () => {
        ...
        const [proposalRes, statsRes] = await Promise.all([
            getAllProposals(filterStatus === "ALL" ? undefined : filterStatus),
            getProposalStats(),
        ])
    ...
    const handleReject = async (id: string) => {
        ...
        setActionLoading(id)
        const result = await rejectProposal(id, notes)
        if (result.success) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div>
                <h1 className="text-3xl font-bold text-slate-800">Kelola Proposal</h1>
                <p className="text-slate-500">Tinjau dan berikan keputusan atas proposal yang sudah diverifikasi.</p>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {([
                    { key: "submitted", label: "Baru", icon: FiClock, color: "text-blue-600 bg-blue-50" },
                    { key: "verified", label: "Terverifikasi", icon: FiCheckCircle, color: "text-indigo-600 bg-indigo-50" },
                    { key: "revision", label: "Revisi", icon: FiClock, color: "text-amber-600 bg-amber-50" },
                    { key: "approved", label: "Disetujui", icon: FiCheckCircle, color: "text-green-600 bg-green-50" },
                    { key: "rejected", label: "Ditolak", icon: FiXCircle, color: "text-red-600 bg-red-50" },
                ] as const).map(s => {
                    const Icon = s.icon
                    return (
                        <div key={s.key} className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
                            ...
                            <p className="text-2xl font-bold text-slate-800">{stats[s.key]}</p>
                        </div>
                    )
                })}
            </div>
                            ...
                            <div key={p.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                ...
                                {isExpanded && (
                                    <div className="px-6 pb-6 border-t border-slate-100 pt-4 space-y-4 animate-in slide-in-from-top-2">
                                        ...
                                        {p.operatorNotes && <div className="text-sm p-3 bg-indigo-50 border border-indigo-200 rounded-xl"><span className="font-semibold text-indigo-700">Catatan Operator: </span>{p.operatorNotes}</div>}

                                        {/* Actions — only for VERIFIED proposals */}
                                        {p.status === "VERIFIED" && (
                                            <div className="pt-4 border-t border-slate-100 space-y-3">
                                                <button
                                                    onClick={() => handleApprove(p.id)}
                                                    disabled={actionLoading === p.id}
                                                    className="px-6 py-2.5 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm flex items-center gap-2"
                                                >
                                                    {actionLoading === p.id ? <FiLoader className="animate-spin" /> : <FiCheckCircle />} Setujui Proposal
                                                </button>
                                                <div className="flex gap-2">
                                                    <input
                                                        ...
                                                        className="flex-1 px-4 py-2.5 border rounded-xl bg-slate-50 text-sm focus:ring-2 focus:ring-red-100 outline-none"
                                                    />
                                                    <button
                                                        onClick={() => handleReject(p.id)}
                                                        disabled={actionLoading === p.id}
                                                        className="px-5 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm flex items-center gap-2 shrink-0"
                                                    >
                                                        {actionLoading === p.id ? <FiLoader className="animate-spin" /> : <FiXCircle />} Tolak
                                                    </button>
                                                </div>
                                            </div>
                                        )}
